...
const migration003 = fs.readFileSync(
  path.join(
    process.cwd(),
    "supabase/migrations/003_activation_hardening.sql",
  ),
  "utf8",
);
...
describe("segurança e evolução do banco", () => {
  it("mantém ownership de mensagens e configurações via RLS", () => {
  ...
  it("isola anexos e buckets privados por usuário", () => {
  ...
  it("adiciona idempotência sem apagar tabelas", () => {
    expect(migration003).toContain(
