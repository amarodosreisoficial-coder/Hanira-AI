...
export async function POST(request: Request) {
  ...
  if (!tryAcquireConcurrencyLock(user.id, `image:${requestId}`)) return error("generation_unavailable", 409);
  ...
  try {
    ...
    if (!result.success || !result.imageData || !result.mimeType) return error(result.errorCode === "capacity_unavailable" || result.errorCode === "rate_limit" ? "capacity_unavailable" : "generation_unavailable", result.errorCode === "capacity_unavailable" || result.errorCode === "rate_limit" ? 503 : 502);
  ...
  } catch { return error("invalid_request", 400); } finally { release(); }
}
